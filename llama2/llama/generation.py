...
import sys
sys.path.append(os.path.join(os.getcwd(), 'ai.architecture', 'genAI'))
...
class Llama:
    ...
    def build(
        ...
        # torch.set_default_dtype(torch.float16) # torch.set_default_tensor_type(torch.cuda.HalfTensor)
    ...
    def generate(
        ...
        for i, cur_pos in enumerate(range(min_prompt_len, total_len)):
            ...
            #torch.save(cache_k[0], os.path.join(save_dir, 'keys_0_off.pt'))
            
            # print(keys[0])
            
            # logits, a, b, c, d = ort_session.run([output.name for output in outputs], input_dict)
            
            # logits, keys[0], keys[1], values[0], values[1] = torch.from_numpy(logits), torch.from_numpy(a), torch.from_numpy(b), torch.from_numpy(c), torch.from_numpy(d)
            
            # logits, cache_k, cache_v = ort_session.run([output.name for output in outputs], {inputs[0].name: tokens[:, prev_pos:cur_pos].numpy(), inputs[1].name: cache_k.numpy(), inputs[2].name: cache_v.numpy()})
            # logits, cache_k, cache_v = torch.from_numpy(logits), torch.from_numpy(cache_k), torch.from_numpy(cache_v)
            
            # torch.save(logits, 'logits_onnx.pt')
