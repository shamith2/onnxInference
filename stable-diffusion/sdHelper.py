...
def createVideofromFrames(
    ...
    video = cv2.VideoWriter(os.path.join(images_dir, filename + '.mp4'), cv2.VideoWriter_fourcc(*'mp4v'), 30, (width, height))
...
def saveTensorasImage(
    ...
    with open(os.path.join(result_dir, 'int_shape.log'), 'a') as f:
        f.write("Min: {} Max: {}\n".format(numpy.min(tensor), numpy.max(tensor)))
...
def visualizeLatents(
    ...
    image_files = [os.path.join(workspace, img) for img in os.listdir(workspace) if img.endswith('.png') and 'int' in img]
    ...
    for i, (ax, img_path) in enumerate(zip(axes, sorted(image_files, reverse=True))):
    ...
    path = os.path.join(workspace, title + '_visualize.png')
